    ...
    function Window:MakeTab(t)
        ...
        function Tab:AddSection(s)
            ...
            function Section:AddKeybind(data)
                ...
                local row = Instance.new("Frame", _G.ignore and nil or self._body)
                row.Size = UDim2.new(1,0,0,32)
                row.BackgroundTransparency = 1
                ...
                local function keyToEnum(v)
                    if typeof(v) == "EnumItem" and v.EnumType == Enum.KeyCode then return v end
                    if typeof(v) == "string" then return Enum.KeyCode[v] or NONE end
                    return NONE
                end

                local initial = data.Default
                if typeof(initial) == "EnumItem" and initial.EnumType == Enum.KeyCode then
                    initial = initial.Name
                end
                if initial == nil then initial = "Unknown" end

                local key = keyToEnum(self._window._save and (UILib.Configs[data.Name] or initial) or initial)

                local binding = false
                local function setLabel()
                    btn.Text = "[" .. (key ~= NONE and key.Name or "None") .. "]"
                end
                local function setKey(k)
                    key = keyToEnum(k)
                    if self._window._save then
                        UILib.Configs[data.Name] = key.Name
                        writejson(self._window._cfgpath, UILib.Configs)
                    end
                    setLabel()
                end
